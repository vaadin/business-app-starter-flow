...
public class Catalogue extends SplitViewFrame {
    ...
    private AppBar createAppBar() {
        ...
        appBar.setSearchPlaceholder(
                "Search by item name, description and vendor");
        return appBar;
    }
    ...
    private Grid createGrid() {
        ...
        grid.addColumn(new ComponentRenderer<>(UIUtils::createBadge))
                .setHeader("Category").setWidth(UIUtils.COLUMN_WIDTH_M)
                .setFlexGrow(0);
        grid.addColumn(new ComponentRenderer<>(this::createInfo))
                .setHeader("Item").setWidth(UIUtils.COLUMN_WIDTH_XL)
                .setFlexGrow(1);
        grid.addColumn(Item::getVendor).setHeader("Vendor")
                .setWidth(UIUtils.COLUMN_WIDTH_XL).setFlexGrow(1);
        grid.addColumn(new ComponentRenderer<>(this::createPrice))
                .setHeader("Unit Price ($)").setWidth(UIUtils.COLUMN_WIDTH_S)
                .setFlexGrow(0).setTextAlign(ColumnTextAlign.END);
        grid.addColumn(new ComponentRenderer<>(this::createStock))
                .setHeader("In Stock").setWidth(UIUtils.COLUMN_WIDTH_S)
                .setFlexGrow(0).setTextAlign(ColumnTextAlign.END);
        grid.addColumn(new ComponentRenderer<>(this::createSold))
                .setHeader("Units Sold").setWidth(UIUtils.COLUMN_WIDTH_S)
                .setFlexGrow(0).setTextAlign(ColumnTextAlign.END);
    ...
    private void filter(HasValue.ValueChangeEvent event) {
        dataProvider.setFilter((SerializablePredicate<Item>) item -> {
            // TODO: This is just for demo purposes. Proper filtering should be
            // done on another level.
            String data = (item.getName() + item.getDesc() + item.getVendor())
                    .toLowerCase();
            if (data.toLowerCase()
    ...
    private Component createDetails(Item item) {
        ...
        category.setRenderer(
                new ComponentRenderer<>(i -> UIUtils.createBadge(i)));
