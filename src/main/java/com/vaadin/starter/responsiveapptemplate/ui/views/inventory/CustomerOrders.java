...
public class CustomerOrders extends SplitViewFrame {
    ...
    private Grid createGrid() {
        ...
        grid.setItemDetailsRenderer(
                new ComponentRenderer<>(this::createDetails));
        grid.setSizeFull();
        ...
        grid.addColumn(new LocalDateRenderer<>(Order::getDate,
                DateTimeFormatter.ofPattern("MMM dd, YYYY")))
                .setComparator(Order::getDate).setFlexGrow(0)
                .setHeader("Order Received").setWidth(UIUtils.COLUMN_WIDTH_M);
        grid.addColumn(new ComponentRenderer<>(this::createItemCount))
                .setFlexGrow(0).setHeader("Item Count")
                .setWidth(UIUtils.COLUMN_WIDTH_S)
                .setTextAlign(ColumnTextAlign.END);
        grid.addColumn(new ComponentRenderer<>(this::createValue))
                .setFlexGrow(0).setHeader("Value ($)")
                .setWidth(UIUtils.COLUMN_WIDTH_S)
                .setTextAlign(ColumnTextAlign.END);
    ...
    private Component createDetails(Order order) {
        ...
        return new Div(shippingTitle, shippingItem,
                new Divider(FlexComponent.Alignment.END, LumoStyles.Space.S),
                trackingTitle, trackingItem,
                new Divider(FlexComponent.Alignment.END, LumoStyles.Space.S),
                handlerTitle, handlerItem,
                new Divider(FlexComponent.Alignment.END, LumoStyles.Space.S),
                itemsTitle, items);
    }
    ...
    private ListItem createHandlerItem() {
        ...
        FlexBoxLayout buttons = new FlexBoxLayout(new Button("Call"),
                new Button("Email"));
        buttons.setSpacing(Right.S);

        ListItem item = new ListItem(initials, handler.getName(),
                handler.getEmail(), buttons);
        item.setHorizontalPadding(false);
        return item;
    }
    ...
    private Div createItems(Order order) {
        Div items = UIUtils
                .createDiv(Collections.singleton(LumoStyles.Margin.Bottom.M));
    ...
    private FormLayout createDialogForm() {
        ...
        items.addValueChangeListener(e -> {
            ...
            if (item != null) {
                ...
                badge.addClassNames(LumoStyles.Margin.Right.S,
                        LumoStyles.Margin.Vertical.XS);
                badges.add(badge);
                amount.setText(UIUtils.formatAmount(
                        list.stream().mapToDouble(Item::getPrice).sum()));
            }
        });
    ...
    private void filter(HasValue.ValueChangeEvent event) {
        ...
        // on another level.
        dataProvider.setFilter((SerializablePredicate<Order>) order -> order
