...
public class ReportDetails extends ViewFrame implements HasUrlParameter<Long> {
    ...
    public void setParameter(BeforeEvent beforeEvent, Long id) {
        ...
        runningDate.setPrimaryText(UIUtils.formatDate(report.getStartDate())
                + " - " + UIUtils.formatDate(report.getEndDate()));
    ...
    private AppBar createAppBar() {
        ...
        appBar.getContextNaviIcon()
                .addClickListener(e -> UI.getCurrent().navigate("reports"));
        return appBar;
    }

    private Component createContent() {
        FlexBoxLayout content = new FlexBoxLayout(createLogoSection(),
                createHeader("Accounts (USD)"), createAccounts(),
                createHeader("Pending Events"), createPendingEvents(),
    ...
    private FlexBoxLayout createLogoSection() {
        ...
        image.addClassNames(LumoStyles.Margin.Horizontal.L,
                LumoStyles.Margin.Bottom.S);
        image.setHeight("200px");
        image.setWidth("200px");

        balance = new ListItem(UIUtils.createTertiaryIcon(VaadinIcon.MONEY), "",
                "Current Balance");
        balance.setDividerVisible(true);
        balance.setReverse(true);

        runningDate = new ListItem(
                UIUtils.createTertiaryIcon(VaadinIcon.CALENDAR), "",
                "Date Range");
        runningDate.setDividerVisible(true);
        runningDate.setReverse(true);

        status = new ListItem(UIUtils.createTertiaryIcon(VaadinIcon.LOCK), "",
                "Status");
        status.setReverse(true);

        FlexLayout listItems = UIUtils.createColumn(balance, runningDate,
                status);
    ...
    private Label createHeader(String title) {
        ...
        header.addClassNames(LumoStyles.Margin.Bottom.M,
    ...
    private FlexLayout createAccounts() {
        ...
        accounts = UIUtils.createWrappingFlexLayout(
                Arrays.asList(LumoStyles.Padding.Bottom.L,
                        BoxShadowBorders.BOTTOM),
                createLargeListItem(VaadinIcon.PLUS,
                        UIUtils.formatAmount(amount * 0.4), "14 deposits"),
                createLargeListItem(VaadinIcon.MINUS,
                        UIUtils.formatAmount(amount * 0.6), "9 withdrawals"),
                createLargeListItem(VaadinIcon.PLUS_MINUS,
                        UIUtils.formatAmount(amount), "23 in total"));
        return accounts;
    }
    ...
    private FlexLayout createPendingEvents() {
        events = UIUtils.createWrappingFlexLayout(
                ...
                createLargeListItem(VaadinIcon.TIMER,
                        UIUtils.formatAmount(DummyData.getRandomInt(0, 50)),
                        "Open"),
                createLargeListItem(VaadinIcon.CHECK,
                        UIUtils.formatAmount(DummyData.getRandomInt(0, 100)),
                        "Closed"),
                createLargeListItem(VaadinIcon.BAN,
                        UIUtils.formatAmount(DummyData.getRandomInt(0, 50)),
                        "Failed"));
        return events;
    }

    private ListItem createLargeListItem(VaadinIcon icon, String primary,
            String secondary) {
        ...
        if (icon.equals(VaadinIcon.CHECK)
                || icon.equals(VaadinIcon.FLAG_CHECKERED)) {
    ...
    private Component createTransactionsChart() {
        ...
        xAxis.setCategories("14.9.", "15.9.", "16.9.", "17.9.", "18.9.",
                "19.9.", "20.9.", "21.9.");
        conf.addxAxis(xAxis);

        conf.getyAxis().setTitle("");

        conf.addSeries(new ListSeries("Withdrawals", 220, 100, 400, 360, 60,
                660, 740, 800));
        conf.addSeries(
                new ListSeries("Deposits", 400, 300, 0, 440, 480, 400, 0, 0));
